...
// xupx 1420 end 
	...
	{
		pDevice->m_StrobeOnOff = 1;
		pDevice->m_FrameFrozen = 0;
		pDevice->m_Brightness = 16;
		
		if (IS_SV1400(pDevice->m_pInfo->DeviceType()))
		{
			pDevice->m_PacketSize=1400;
			pDevice->m_pShutter->Set(120);
		}

		else if (IS_SV1410(pDevice->m_pInfo->DeviceType())||IS_SV1420(pDevice->m_pInfo->DeviceType()))
		{
			pDevice->m_PacketSize=2800;
			pDevice->m_pShutter->Set(60);
		}
		else if (IS_SV2000(pDevice->m_pInfo->DeviceType()))
		{
			pDevice->m_PacketSize = 3600;
			pDevice->m_pShutter->Set(70);
		}
		...
		else if (IS_SV1300(pDevice->m_pInfo->DeviceType()))
		{
			pDevice->m_PacketSize=1600;
			pDevice->m_pShutter->Set(133);
		}
		else
		{
			pDevice->m_PacketSize=2500;
			pDevice->m_pShutter->Set(60);
		}
		
		if (!IS_NOT_SV_COLOR(pDevice->m_pInfo->DeviceType()))
		{
			pDevice->m_BlueGain=28;
		    pDevice->m_RedGain =14;
			pDevice->Set_WB_Red(14);
		    pDevice->Set_WB_Blue(28);
		}
			...
			IS_SV400(pDevice->m_pInfo->DeviceType()))
		{
			...
			pDevice->m_bRaw8or12=0;
			pDevice->m_bLeBe = 1;
			pDevice->m_bTriggerDelayOnOff=0;
			pDevice->m_nTriggerDelayValue=1;
			pDevice->m_nTransfersDelayValue=0;
			pDevice->m_nFilterTimeUnit = 0;
			pDevice->m_nFilterTime = 10;
		...
		pDevice->SetShutterUnit(1);
		pDevice->m_pGain->Set(296);

		pDevice->Set_Brightness();
		
		pDevice->Set_Frame_Frozen(0);
		...
		pDevice->ADCControl(ADC_BITS, (HV_ADC_LEVEL)4);//查找表
		pDevice->m_LutPath="";
...
//CMOS
    if (IS_CMOS_1394_CAM(pDevice->m_pInfo->DeviceType())) 
	{
		...
		if (IS_HV2000(pDevice->m_pInfo->DeviceType()))
		{
		  pDevice->m_pShutter->Set(100);
		}
		if (IS_HV3100(pDevice->m_pInfo->DeviceType()))
		{
		  pDevice->m_pShutter->Set(152);
		}
		pDevice->m_pGain->Set(8);
		pDevice->m_pADCLevel->Set(3);
		pDevice->m_pBlackLevelEnable->Set(1);
		pDevice->m_pBlackLevel->Set(-20);
        pDevice->m_Resolution=VIDEO_MODE0;
		pDevice->SetVideoMode(VIDEO_MODE0);
    }
