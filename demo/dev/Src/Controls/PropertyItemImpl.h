...
public:
   ...
   virtual void SetCustomData(int value)
   {
       m_CustomData = value;
   }
   virtual ~CProperty()
...
class CPropertyTrackItem : public CPropertyEditItem
{
public:
   CPropertyTrackItem(LPCTSTR pstrName, TRACKBARINFO trackinfo, LPARAM lParam) : 
      CPropertyEditItem(pstrName, lParam),
   ...
   HWND CreateInplaceControl(HWND hWnd, const RECT& rc) 
   {
      ...
      if( !GetDisplayValue(pszText, cchMax) ) return NULL;
      // Create window
      CPropertyTrackWindow* pWnd = new CPropertyTrackWindow();
      ATLASSERT(pWnd);
      RECT rcWin = rc;
      m_hwndEdit = pWnd->Create(hWnd, rcWin, pszText);
	  // Reset track bar range 
	  pWnd->m_wndTrack.SetRange(m_trackinfo.RangeMin, m_trackinfo.RangeMax);
	  pWnd->m_wndTrack.SetLineSize(m_trackinfo.LineSize);//added by HYL 2007.02.07
   ...
   virtual BOOL SetRange(int nMin, int nMax)
   {
	   m_trackinfo.RangeMin = nMin;
	   m_trackinfo.RangeMax = nMax;
	   
	   return 1;
   }
   BOOL SetValue(HWND hWnd)
   {
		ATLTRACE(" CPropertyTrackItem :SetValue");
      return CPropertyEditItem::SetValue(hWnd);
   }

private:
	TRACKBARINFO m_trackinfo;
};
   ...
   BOOL Activate(UINT action, LPARAM /*lParam*/)
   {
      ...
      case PACT_DBLCLICK:
         // Let control owner know
		  CFileDialog dlg(TRUE, _T("Lut"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Lut Files (*.lut)\0*.lut\0All Files (*.*)\0*.*\0"), ::GetParent(m_hWndOwner));
          if(dlg.DoModal() == IDOK) {
              CComVariant vValue = dlg.m_szFileName;
              SetValue(vValue);
          }
...
class CPropertyEditButtonItem : public CPropertyItem
{
public:
	LPCTSTR m_pstrFilter;
	CPropertyEditButtonItem(LPCTSTR pstrName, LPARAM lParam = 0) : CPropertyItem(pstrName, lParam)
	{
	}  
	HWND CreateInplaceControl(HWND hWnd, const RECT& rc) 
	{
		...
		if( !GetDisplayValue(szText, (sizeof(szText) / sizeof(TCHAR)) - 1) ) return NULL;      
		// Create EDIT control
		CPropertyStandardButtonWindow* win = new CPropertyStandardButtonWindow();
		ATLASSERT(win);
		RECT rcWin = rc;
		win->m_prop = this;
		win->Create(hWnd, rcWin, szText);
...
inline HPROPERTY PropCreateEditButton(LPCTSTR pstrName, LPCTSTR pstrValue, LPARAM lParam = 0)
{
	ATLASSERT(!::IsBadStringPtr(pstrValue,-1));
	CPropertyEditButtonItem* prop = NULL;
	//  yanshuai modify 07.07.21
	//   ATLTRY( prop = new CPropertyFileNameItem(pstrName, lParam) );
	ATLTRY( prop = new CPropertyEditButtonItem(pstrName, lParam) );   
	//  end yanshuai modify 07.07.21
	ATLASSERT(prop);
	if( prop == NULL ) return NULL;
	CComVariant vValue = pstrValue;
	prop->SetValue(vValue);
...
inline HPROPERTY PropCreateTrack(LPCTSTR pstrName, int iValue, TRACKBARINFO trackinfo, LPARAM lParam = 0)
{
   IProperty* prop = NULL;
   ATLTRY( prop = new CPropertyTrackItem(pstrName, trackinfo, lParam) );
   ATLASSERT(prop);
   if( prop == NULL ) return NULL;
   CComVariant vValue = iValue;
   prop->SetValue(vValue);
