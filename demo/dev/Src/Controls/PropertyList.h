...
public:
   ...
   BEGIN_MSG_MAP(CPropertyListImpl)
	  ...
	  MESSAGE_HANDLER(WM_USER_PROP_SCALARPROPERTY, OnScalarProperty);
   ...
   {      
	  ATLTRACE("CPropertyListImpl::  OnScroll\n");
      _DestroyInplaceWindow();
      bHandled = FALSE;
      return 0;
   }
   ...
   {
	  ATLTRACE("CPropertyListImpl::  OnKeyDown\n");
   ...
   {      
	  ATLTRACE("CPropertyListImpl::  OnChar\n");
   ...
   {
	  ATLTRACE("CPropertyListImpl::  OnNavigate\n");
      switch( wParam ) {
      case VK_UP:
      case VK_DOWN:
         {
   ...
   {
	  ATLTRACE("CPropertyListImpl::  OnUpdateProperty\n");
   ...
   {
	  ATLTRACE("CPropertyListImpl::  OnChangedProperty\n");
   ...
   LRESULT OnScalarProperty(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
   {
      // Updates a property value.
      // A property class uses this message to make sure the corresponding editor window
      // is updated as well.
      ...
      // Ask owner about change
      NMPROPERTYSCALAR nmh = { m_hWnd, GetDlgCtrlID(), PIN_SCALARCHANGED, prop, value };   
      ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
	  if( !prop->SetValue(hWnd) ) ::MessageBeep((UINT)-1);
      return 0;
   }
