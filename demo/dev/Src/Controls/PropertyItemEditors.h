...
/////////////////////////////////////////////////////////////////////////////
// Editor with trackbar dropdown

class CPropertyTrackWindow : 
   public CPropertyDropWindowImpl<CPropertyTrackWindow>
{
public:
   DECLARE_WND_SUPERCLASS(_T("WTL_InplacePropertyTrackEdit"), CEdit::GetWndClassName())

   CContainedWindowT<CTrackBarCtrl> m_wndTrack;

   typedef CPropertyDropWindowImpl<CPropertyTrackWindow> baseClass;

   
   BEGIN_MSG_MAP(CPropertyTrackWindow)
      MESSAGE_HANDLER(WM_CREATE, OnCreate)
      MESSAGE_HANDLER(WM_DESTROY, OnDestroy)
	  MESSAGE_HANDLER(WM_KEYDOWN, OnKeyDown)
      MESSAGE_HANDLER(WM_DRAWITEM, OnDrawItem)
      ...
      COMMAND_CODE_HANDLER(BN_CLICKED, OnButtonClicked)
	  NOTIFY_CODE_HANDLER(NM_RELEASEDCAPTURE, OnReleasedCapture)
      CHAIN_MSG_MAP( baseClass )
   ALT_MSG_MAP(1) // Button
      CHAIN_MSG_MAP_ALT( baseClass, 1 )
   ALT_MSG_MAP(2) // Track
   ...
   {
      LRESULT lRes = baseClass::OnCreate(uMsg, wParam, lParam, bHandled);
      // Create dropdown track bar (as hidden)
      m_wndTrack.Create(this, 2, m_hWnd, &rcDefault, NULL, WS_POPUP |TBS_NOTICKS | TBS_TOP );
      ATLASSERT(m_wndTrack.IsWindow());
      m_bReadOnly = false;
      return lRes;
   }
   LRESULT OnDestroy(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
   {
      if( m_wndTrack.IsWindow() ) m_wndTrack.DestroyWindow();
      bHandled = FALSE;
      return 0;
   }
   LRESULT OnKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
	  if ( wParam == VK_RETURN) {
		  ...
		  GetWindowText(szPos, (sizeof(szPos) / sizeof(TCHAR)) - 1);
		  value = atoi(szPos);
		  m_wndTrack.GetRange(min, max);
		  int nLs=m_wndTrack.GetLineSize();//added by HYL 2007.02.07
		  if (value < min){
			  value = min;
		  }
		  else if (value > max){
		  ...
		  itoa(value, szPos, 10);
		  SetWindowText(szPos);
		  ::SendMessage(GetParent(), WM_USER_PROP_SCALARPROPERTY, value, (LPARAM) m_hWnd);	
	  }
   ...
   LRESULT OnButtonClicked(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
   {
	  ...
	  TCHAR szPos[64] = { 0 };
      GetWindowText(szPos, (sizeof(szPos) / sizeof(TCHAR)) - 1);
      m_wndTrack.SetPos(atoi(szPos));
	
      // Move the track bar under the item
      RECT rcTrack = {0, 0, 100, 15};
      RECT rcWin;
      GetWindowRect(&rcWin);
      ::OffsetRect(&rcTrack, rcWin.left, rcWin.bottom);
      m_wndTrack.SetWindowPos(HWND_TOPMOST, &rcTrack, SWP_SHOWWINDOW);
      m_wndTrack.SetFocus();
      return 0;
   }
   ...
   {//Modified by Hyl 2007.02.07
	  ...
	  int nLs=m_wndTrack.GetLineSize();
   ...
   // Trackbar message handler

   LRESULT OnKillFocus(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      LRESULT lRes = m_wndTrack.DefWindowProc();
      m_wndTrack.ShowWindow(SW_HIDE);  
      return lRes;
   }

   // Overdrawn button message handler
...
/////////////////////////////// add by xupx 2009.07.01 //////////////////////////////////////////////
// Editor with Standard button
   class CPropertyStandardButtonWindow : 
   public CPropertyDropWindowImpl<CPropertyButtonWindow>
   {
   public:
	   DECLARE_WND_SUPERCLASS(_T("WTL_InplacePropertyButton"), CEdit::GetWndClassName())
	   ...
	   BEGIN_MSG_MAP(CPropertyButtonWindow)
		   COMMAND_CODE_HANDLER(BN_CLICKED, OnButtonClicked)
		   MESSAGE_HANDLER(WM_DRAWITEM, OnDrawItem)
		   MESSAGE_HANDLER(WM_CREATE, OnCreate)		// add by xupx 2009.07.01
		   CHAIN_MSG_MAP( baseClass )
		   ALT_MSG_MAP(1) // Button
		   CHAIN_MSG_MAP_ALT( baseClass, 1 )
		   END_MSG_MAP()
	   ...
	   LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)	//2009.07.01 by xupx
	   {
		   LRESULT lRes = DefWindowProc(uMsg, wParam, lParam);
		   RECT rcClient;
		   GetClientRect(&rcClient);
		   ...
		   SendMessage(EM_SETMARGINS, EC_LEFTMARGIN|EC_RIGHTMARGIN, MAKELONG(PROP_TEXT_INDENT, ::GetSystemMetrics(SM_CXVSCROLL)));
		   // Create button
		   RECT rcButton = { rcClient.left, rcClient.top - 1, rcClient.right, rcClient.bottom };
