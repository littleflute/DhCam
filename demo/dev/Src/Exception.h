...
{
public:
  Exception( ErrorNumber e, CString context = "", va_list *pArgs = NULL );
...
inline void Exception::FormatMessage( const CString& LibraryFile, char** ppArgs )
{
  ...
  m_Description = "<no message found>";
  ...
  if ( cCopied != 0 )
  {
    ...
    if ( ppArgs != NULL )
    {
      ...
      catch (...)
      {
        ...
        m_Description += "<unable to format message>";
        assert( false && "<unable to format message>" );
      }
      if ( cCopied != 0 )
      {
        m_Description = (LPCSTR) lpFormattedMsg;
