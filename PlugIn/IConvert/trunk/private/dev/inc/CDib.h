...
/// 操作模式
typedef enum tagOPERATE_MODE
{
		OP_IDLE				=  0,
		OP_TRANSFER			=  1,	//移动图像
		OP_ZOOM_UP			=  2,	//放大图像
		OP_ZOOM_DOWN		=  3,	//缩小图像
		OP_REGION2PANE		=  4,	//局部填充整个区域
		OP_MAGNIFIER		=  5,	//放大镜
		OP_FIT				=  6,
...
/// 状态返回码
typedef enum tagCSSTATUS
{
		STATUS_IMG_OK					= 0,
		STATUS_IMG_NOT_ENOUGH_MEM		= -1,
		STATUS_IMG_CREATE_WND_FAILED	= -2,
		STATUS_IMG_PARAM_INVAILD		= -3,
		STATUS_IMG_LOAD_IMG_FAILED		= -4,
		STATUS_IMG_SAVE_IMG_FAILED		= -5,

		STATUS_IMG_UNAVAIL				= -10,
...
/**
* \brief   SCImage: 保存图像信息以及图像数据.
*
* 根据图像信息，为图像数据开辟内存，用于存放图像数据。
*
* 
*/
class AFX_EXT_CLASS SCImage
{
public:
	SCImage();
	
	bool Create(CSize& ImageSize, int bitDepth, CPoint& Origin);
	...
	CRect GetRect() { return CRect(m_Origin, m_Size); }
	...
	operator CDib&() { return m_Bitmap; }
	
	operator const CDib&() const { return m_Bitmap; }
