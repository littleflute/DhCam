...
HVSTATUS __stdcall HVGetDeviceGUID(GUID *pGUID, DWORD cb, DWORD *cbNeeded);
HVSTATUS __stdcall HVGetTypeFromIndex(int nDevice, HVTYPE *pType, int *pIndex);
HVSTATUS __stdcall HVGetDevInterfaceFromIndex(int nDevice, HV_DEVICE_INTERFACE *pDeviceInterface);

HVSTATUS __stdcall BeginHVDeviceEx(void* p,HV_BEGIN_TYPE type, HHV *pHandle);
