...
void CDlgSetParameter::OnShowFrame(CConvertImageInfo *pImageInfo, CConvertImageData *pImageData)
{
	//v1.1
	m_pParam.m_info = *pImageInfo;
	m_pParam.m_data = *pImageData;
	m_nNowDepth = pImageInfo->ciiDest.iV11DataDepth;
	m_ImageView.m_nNowDepth = m_nNowDepth;	// add by xupx 2008.09.27

	//Is the colorcode supported or not?
	if (!isSupportedColorCode(pImageInfo->ciiSrce.iV11ColorCode))
	{
		if ( IsWindowVisible() )
			//SendMessage(WM_SETPARAMDLG_UPDATE, 0, 0);
			OnUpdateView(0,0);		//by lm 20090709

		return;
	}	//add by lm 20090707

	if( pImageInfo->ciiSrce.iV11ColorCode != COLOR_MONO8 &&
		pImageInfo->ciiSrce.iV11ColorCode != COLOR_RAW8  &&
		pImageInfo->ciiSrce.iV11ColorCode != COLOR_JPEG) //add by lm 20090706
	{
		if(pImageInfo->ciiSrce.iV11ColorCode == COLOR_RAW12_PACKED)
		{
		...
		else if(pImageInfo->ciiSrce.iV11ColorCode == COLOR_RAW10_PACKED ||//hj 091014
			    pImageInfo->ciiSrce.iV11ColorCode == COLOR_MONO10_PACKED) //hj 091020
		{ 
			BYTE* pTemp = new BYTE[pImageInfo->ciiDest.iV11Width * pImageInfo->ciiDest.iV11Height * 2];
		...
		{
			RawDatato8((BYTE *)pImageData->pCidSrce, 
				   (BYTE *)pImageData->pCidSrce, 
				   pImageInfo->ciiDest.iV11Width,
				   pImageInfo->ciiDest.iV11Height, 
				   m_CurrentShift);
		}
	...
	if (pImageInfo->ciiSrce.iV11ColorCode == COLOR_JPEG)	
	{
		...
		if (hDib != NULL) 
		{
			...
			unsigned char* pTmpBuffer = (unsigned char* )(pDIB + sizeof(BITMAPINFOHEADER));

			memcpy(pImageData->pCidDest, pTmpBuffer,pImageInfo->ciiDest.iV11SizeImage );
	...
	if ( IsWindowVisible() )
	{
		if (pImageInfo->ciiSrce.iV11ColorCode != COLOR_JPEG)	//add by lm 20090706
		{
			// White balance
			if (m_bBalance == TRUE)
			{
				WhiteBalance(pImageData->pCidDest, pImageData->pCidSrce, pImageInfo->ciiDest.iV11Width, pImageInfo->ciiDest.iV11Height);
			}
	...
	else
	{
		if (pImageInfo->ciiSrce.iV11ColorCode != COLOR_JPEG)	//add by lm 20090706
		{
			if (m_bBalance == TRUE && m_ImageView.m_nNowDepth != 8)		// add by xupx 2008.09.05. when camera is snapping, WB is done.
			{
				WhiteBalance(pImageData->pCidDest, 
					pImageData->pCidSrce, 
					pImageInfo->ciiDest.iV11Width,
					pImageInfo->ciiDest.iV11Height);
				m_pParam.SaveParam(m_KeyPath);
				m_sParam = m_pParam;
				m_bBalance = FALSE;		// add by xupx
			}
			else if(m_bSavePara && m_ImageView.m_nNowDepth == 8)	// add by xupx 2008.10.06, 因为，相机切换时，一台为8位显示会死程序
			{
				m_pParam.UpdateLutGama();
				m_pParam.SaveParam(m_KeyPath);
				m_sParam = m_pParam;
