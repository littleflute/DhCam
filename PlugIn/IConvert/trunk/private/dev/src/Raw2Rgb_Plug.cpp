...
extern "C" void __stdcall PluginOnManagerNotify(HANDLE handle, CConvertImageInfo *pImageInfo)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState);
	CPluginModule *pPlugin = (CPluginModule *)handle;
	if (pPlugin == NULL )
...
extern "C" void __stdcall PluginOnStartGrabbing(HANDLE handle, CConvertImageInfo *pImageInfo)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState);
	CPluginModule *pPlugin = (CPluginModule *)handle;
	if (pPlugin == NULL )
...
extern "C" void __stdcall PluginOnStopGrabbing(HANDLE handle,CConvertImageInfo *pImageInfo, CConvertImageData *pImageData)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState);
	CPluginModule *pPlugin = (CPluginModule *)handle;
	if (pPlugin == NULL )
...
extern "C" void __stdcall PluginOnShowFrame(HANDLE handle, CConvertImageInfo *pImageInfo, CConvertImageData *pImageData)
{
	CPluginModule *pPlugin = (CPluginModule *)handle;
	if (pPlugin == NULL )
		return ;
