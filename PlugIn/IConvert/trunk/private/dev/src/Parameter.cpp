...
void CParameter::ReadParam(CString keyPath)
{
	try
	{
		...
		m_info.ciiDest.iV11DataDepth = ptrBag->ReadLong("Depth");

		m_Brightness  = ptrBag->ReadLong("Brightness");
		m_Contrast    = ptrBag->ReadLong("Contrast");
		m_Sharpen	  = ptrBag->ReadFloat("Sharpen");
...
void CParameter::SaveParam(CString keyPath)
{
	try
	{
		...
		ptrBag->WriteLong("Depth", m_info.ciiDest.iV11DataDepth);
		ptrBag->WriteLong("Brightness",  m_Brightness);
		ptrBag->WriteLong("Contrast",	 m_Contrast);
		ptrBag->WriteFloat("Sharpen",	 m_Sharpen);
	}
	catch (CPropertyBagException) {
	}
