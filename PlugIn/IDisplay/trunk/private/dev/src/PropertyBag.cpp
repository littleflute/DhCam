...
#define		STATUS_PARAMETER_INVALID 101
...
void CPropertyBag::WriteBool(const CString& name, const BOOL value)
{
  ...
  if ( error != 0 )
    throw CPropertyBagException(error, "CPropertyBag::WriteBool()");
}
...
BOOL CPropertyBag::ReadBool(const CString& name)
{
  ...
  {
    throw CPropertyBagException(error, "CPropertyBag::ReadBool()");
  }
  if ( buffer.CompareNoCase("false") == 0 )
    ...
    throw CPropertyBagException(STATUS_PARAMETER_INVALID, "CPropertyBag::ReadBool()");
}
...
void CPropertyBag::WriteLong(const CString& name, const long value)
{
  ...
  if ( error = WriteProperty(name, buffer) )
    throw CPropertyBagException(error, "CPropertyBag::WriteLong()");
}
...
long CPropertyBag::ReadLong(const CString& name)
{
  ...
  if ( error = ReadProperty(name, buffer) )
  {
    throw CPropertyBagException(error, "CPropertyBag::ReadLong()");
  }
  long res;
  if ( ! _stscanf(buffer, "%ld", &res) )
    throw CPropertyBagException(STATUS_PARAMETER_INVALID, "CPropertyBag::ReadLong()");
  return res;
}
...
void CPropertyBag::WriteFloat(const CString& name, const float value)
{
  ...
  DWORD error;
  if ( error = WriteProperty(name, buffer ))
    throw CPropertyBagException(error, "CPropertyBag::WriteFloat()");
}
...
float CPropertyBag::ReadFloat(const CString& name)
{
  ...
  if ( error = ReadProperty(name, buffer) ) 
  {
    throw CPropertyBagException(error, "CPropertyBag::ReadFloat()");
  }
  float res;
  if ( ! _stscanf(buffer, "%f", &res) )
    throw CPropertyBagException(STATUS_PARAMETER_INVALID, "CPropertyBag::ReadFloat()");
  return res;
}
...
void CPropertyBag::WriteString(const CString& name, const CString value)
{
  ...
  if ( error = WriteProperty(name, value) )
    throw CPropertyBagException(error, "CPropertyBag::WriteString()");
}
...
m_strSectionName(sectionName)
{
  ...
  {
    if ( exist )
      throw CPropertyBagException( STATUS_INTERNAL_ERROR, "CIniFilePropertyBag::CIniFilePropertyBag()");
    // write an empty section for the BAG
    if ( ! WritePrivateProfileSection(m_strSectionName, "\0\0", m_strFileName) )
      throw CPropertyBagException( GetLastError(), "CIniFilePropertyBag::Create()" );
  }
  else
  {
    if ( ! exist )
...
CPropertyBagPtr CIniFilePropertyBag::Create(const CString& fileName, const CString& bagName)
{
  ...
  if( ! numCharsCopied )
  {
    ...
    if ( hFile == NULL )
      throw CPropertyBagException(ERROR_CANNOT_MAKE, "CIniFilePropertyBag::Create()");
    fclose(hFile);
    numCharsCopied = SearchPath(
      NULL,                 // search path
      (LPCTSTR) fileName,   // file name
...
CPropertyBagPtr CIniFilePropertyBag::Open(const CString& fileName, const CString& bagName)
{
  ...
  if( ! numCharsCopied )
  {
      throw CPropertyBagException( GetLastError(), "CIniFilePropertyBag::Open()" );
  }
...
CRegistryPropertyBag::CRegistryPropertyBag(HKEY parent, const CString& bagName, BOOL create)
{
	...
	if ( !create && !exist )
	{
		throw CPropertyBagException(error, "CRegistryPropertyBag::CRegistryPropertyBag()");
	}
	if ( create && !exist )
	{
