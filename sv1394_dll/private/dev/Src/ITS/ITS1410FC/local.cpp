...
{
    const static PCHAR LibraryName = "ITS1410FC.dll";
    return LibraryName;
}
...
)
{
    ...
    do
    {
        ...
        if (All_Enumerate(nDevice,&devIF,GUID_ITS1410FC))
        {
            nDevice++;
        }
        else
    ...
    *cbNeeded = nDevice * sizeof (HV_DEVICE_INTERFACE);
    ...
    if (nDevice>0)//有至少一个设备存在
    {
        ...
        for (int i = 0; i < nDevice; i++)
        {
            ...
            {
                HV_DEVICE_INTERFACE devIF;
                if (All_Enumerate(i,&devIF,GUID_ITS1410FC))
                {
                    strcpy(pCurrent->szDevicePath, devIF.szDevicePath);
                    pCurrent->ClassGUID = GUID_ITS1410FC;
                    //	pCurrent->Reserved = ITS1410FCTYPE;
...
)
{
    ...
    status = All_Open(DevicePath,phCamera,pGuid,GUID_ITS1410FC);
    return status;
}
