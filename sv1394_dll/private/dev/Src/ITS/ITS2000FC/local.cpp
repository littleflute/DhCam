...
//ITS 0x49 0x54 0x53
//                                                  2000                                      I       T       S        
DEFINE_GUID(GUID_ITS2000FC, 0x20007067, 0x789b, 0x40aa, 0x49, 0x54, 0x53, 0x5e, 0x21, 0x2f, 0x13, 0xca);

PCHAR GetLibraryName()
{
	const static PCHAR LibraryName = "ITS2000FC.dll";
	return LibraryName;
}
...
{ 
	...
	do{
		...
		if(All_Enumerate(nDevice,&devIF,GUID_ITS2000FC))
		{
			nDevice++;
		}
		else
	...
	*cbNeeded = nDevice * sizeof (HV_DEVICE_INTERFACE);
	...
	{ 
		...
		{    
			...
			{
				HV_DEVICE_INTERFACE devIF;
				if(All_Enumerate(i,&devIF,GUID_ITS2000FC)){
					strcpy(pCurrent->szDevicePath, devIF.szDevicePath); 
					pCurrent->ClassGUID = GUID_ITS2000FC;
					//pCurrent->Reserved = ITS2000FCTYPE;
			...
			)
{
	...
	status = All_Open(DevicePath,phCamera,pGuid,GUID_ITS2000FC);
	return status;
}
